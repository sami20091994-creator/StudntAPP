...
class TeacherFinancialsActivity : ThemedActivity() {
...
class TeacherWithdrawalsAdapter(private val list: List<TeacherWithdrawal>) : RecyclerView.Adapter<TeacherWithdrawalsAdapter.WH>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): WH {
        ...
        val tv1 = TextView(parent.context).apply { id = android.R.id.text1; textSize = 16f; setTextColor(Color.parseColor("#2BB673")); setTypeface(null, android.graphics.Typeface.BOLD) }
        val tv2 = TextView(parent.context).apply { id = android.R.id.text2; textSize = 13f; setTextColor(Color.GRAY) }
        val divider = View(parent.context).apply { layoutParams = LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 2).apply { topMargin = 15 }; setBackgroundColor(Color.parseColor("#F4F6F9")) }
...
class TeacherDuesAdapter(private val list: List<TeacherDue>) : RecyclerView.Adapter<TeacherDuesAdapter.VH>() {
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        holder.tvHighlight.setTextColor(Color.parseColor("#4C53A4"))
    }
    override fun getItemCount() = list.size
}
